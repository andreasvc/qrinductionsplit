...
\item Finite-State automaton miniminization, compression
	...
	%cite DFA minimization (see wikipedia)
...
\item Minimum Description Length principle %cite eg. Mitchell? p. 171, section 6.6
...
\item Dump the remaining dependencies in a single fragment, $UF$. 
	This set should be empty if the model is properly designed, ie., it
	should contain no dependencies that apply to some instances but not to
	others.
...
\begin{align*}
samedeps(R, Q_1, Q_2) := \; \forall d, q_1, q_2 \; [ \; 
...
fragments := \{ \; \{ \; dependency&(d, Q_1, Q_2) \; | \; \\
		& dependency(d, q_1, q_2)  \in M \\
		\land \; & isa(q_1, Q_1)  \land isa(q_2, Q_2) \; \} \\
	\; | \; \langle R, Q_1, Q_2 \rangle &\in pivots \lor \langle R, Q_2, Q_1 \rangle \in pivots \; \}
\end{align*}
...
the automatic modeling implementations of \cite{buisman} and
...
\begin{figure}[ht]
\centering
\includegraphics{ts_mono-crop.pdf}
...
\centering
\subfigure{
    \includegraphics[width=0.4\textwidth]{ts_growth-crop.pdf}
}
\subfigure{
    \includegraphics[width=0.4\textwidth]{ts_sizeshade-crop.pdf}
}
\caption{2 modelfragments for growth and size-shade}
\label{ts_frags}
\end{figure}
...
problem of induction in philosophy (Hume, Popper, etc.). %cite?
